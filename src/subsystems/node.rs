...
use crate::components::scale::{Scale, ScaleError};
use std::{error::Error};
...
impl Node {
    ...
    pub async fn dispense(mut scale: Scale,
                          mut motor: ClearCoreMotor,
                          serving: f64,
                          motor_speed: isize,
                          sample_rate: f64,
                          cutoff_frequency: f64,
    ) -> (Scale, ClearCoreMotor, Data) {
        // Instantiate motor handles
        
        //TODO: We should keep methods and functions from depending on hardware that needs to be 
        // connected. That way we can test off-line instead add something like the ClearCoreController
        // to the struct members so that whenever we use the struct we can inject the controller rather
        // than create a controller in methods or functions. Look at how LinearActuator is  implemented
